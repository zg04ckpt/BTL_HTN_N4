...
const bool DEFAULT_PULSE_ENABLED = true;
...
void quay(float degree) {
  ...
  // Always keep pulsed power enabled while rotating
  bool prevPulseEnabled = g_usePulsedPower;
  g_usePulsedPower = true;
...
void di_thang(float targetDistanceCm) {
  ...
  // Always keep pulsed power enabled while moving forward
  bool prevPulseEnabled = g_usePulsedPower;
  g_usePulsedPower = true;
